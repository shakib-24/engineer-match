...
export function CompanySecuritySettings() {
  const [twoFactorEnabled, setTwoFactorEnabled] = useState<boolean>(
    COMPANY_SECURITY_SETTINGS.twoFactor.defaultChecked,
  );
  const [demoMessage, setDemoMessage] = useState<string | null>(null);

  function showDemoNote() {
    setDemoMessage(COMPANY_SETTINGS_DEMO_NOTE);
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 py-4 first:pt-0">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-foreground">
              {COMPANY_SECURITY_SETTINGS.passwordChange.label}
            </p>
            <p className="mt-0.5 text-xs text-muted-foreground">
              {COMPANY_SECURITY_SETTINGS.passwordChange.description}
            </p>
          </div>
          <button
            type="button"
          ...
          >
            {COMPANY_SECURITY_SETTINGS.passwordChange.buttonLabel}
          </button>
        </div>
