...
import { CLOSE_RECRUITMENT_DIALOG_LABELS } from "@/constants/company-jobs";
...
export function DeleteJobDialog({ isOpen, onCancel, onConfirm }: DeleteJobDialogProps) {
  const titleId = useId();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <button
        type="button"
        aria-label={CLOSE_RECRUITMENT_DIALOG_LABELS.cancelLabel}
        onClick={onCancel}
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h2 id={titleId} className="text-base font-semibold text-foreground">
              {CLOSE_RECRUITMENT_DIALOG_LABELS.title}
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              {CLOSE_RECRUITMENT_DIALOG_LABELS.description}
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {CLOSE_RECRUITMENT_DIALOG_LABELS.cancelLabel}
          </button>
          <button
            type="button"
          ...
          >
            {CLOSE_RECRUITMENT_DIALOG_LABELS.confirmLabel}
          </button>
        </div>
      </div>
    </div>
