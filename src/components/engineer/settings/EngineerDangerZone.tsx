...
import { ENGINEER_DANGER_ZONE, ENGINEER_SETTINGS_DEMO_NOTE } from "@/constants/engineer-settings";
...
function ConfirmDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <button
        type="button"
        aria-label={ENGINEER_DANGER_ZONE.cancelLabel}
        onClick={onCancel}
      ...
      >
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {ENGINEER_DANGER_ZONE.cancelLabel}
          </button>
          <button
            type="button"
...
type DangerAction = "suspend" | "delete" | null;
...
export function EngineerDangerZone() {
  ...
  function handleConfirm() {
    ...
    setToastMessage(ENGINEER_SETTINGS_DEMO_NOTE);
    ...
    <section className="rounded-2xl border border-red-200 bg-red-50/40 p-6 shadow-sm sm:p-8">
      ...
      <div className="mt-6 flex flex-col divide-y divide-red-200">
        <div className="flex flex-wrap items-center justify-between gap-4 py-4 first:pt-0">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-foreground">
              {ENGINEER_DANGER_ZONE.suspend.label}
            </p>
            <p className="mt-0.5 text-xs text-muted-foreground">
              {ENGINEER_DANGER_ZONE.suspend.description}
            </p>
          </div>
          <button
            type="button"
          ...
          >
            {ENGINEER_DANGER_ZONE.suspend.buttonLabel}
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 py-4 last:pb-0">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-foreground">
              {ENGINEER_DANGER_ZONE.delete.label}
            </p>
            <p className="mt-0.5 text-xs text-muted-foreground">
              {ENGINEER_DANGER_ZONE.delete.description}
            </p>
          </div>
          <button
            type="button"
          ...
          >
            {ENGINEER_DANGER_ZONE.delete.buttonLabel}
          </button>
        </div>
      </div>

      <p className="mt-5 text-xs text-red-700/70">{ENGINEER_SETTINGS_DEMO_NOTE}</p>

      <ConfirmDialog
        isOpen={activeDialog === "suspend"}
        title={ENGINEER_DANGER_ZONE.suspend.dialogTitle}
        description={ENGINEER_DANGER_ZONE.suspend.dialogDescription}
        confirmLabel={ENGINEER_DANGER_ZONE.suspend.confirmLabel}
        onCancel={() => setActiveDialog(null)}
        onConfirm={handleConfirm}
      />
      <ConfirmDialog
        isOpen={activeDialog === "delete"}
        title={ENGINEER_DANGER_ZONE.delete.dialogTitle}
        description={ENGINEER_DANGER_ZONE.delete.dialogDescription}
        confirmLabel={ENGINEER_DANGER_ZONE.delete.confirmLabel}
        onCancel={() => setActiveDialog(null)}
        onConfirm={handleConfirm}
      />
