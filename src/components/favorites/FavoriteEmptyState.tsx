...
import {
  FAVORITE_EMPTY_STATE_LABELS,
  FAVORITE_FILTERED_EMPTY_STATE_LABELS,
} from "@/constants/favorites";

interface FavoriteEmptyStateProps {
  variant: "no-favorites" | "no-results";
  onReset?: () => void;
}

export function FavoriteEmptyState({ variant, onReset }: FavoriteEmptyStateProps) {
    ...
    return (
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border bg-surface px-6 py-16 text-center shadow-sm">
        ...
        <p className="text-sm font-semibold text-foreground">
          {FAVORITE_EMPTY_STATE_LABELS.title}
        </p>
        <p className="text-sm text-muted-foreground">
          {FAVORITE_EMPTY_STATE_LABELS.description}
        </p>
        <Link
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border bg-surface px-6 py-16 text-center shadow-sm">
      ...
      <p className="text-sm font-semibold text-foreground">
        {FAVORITE_FILTERED_EMPTY_STATE_LABELS.title}
      </p>
      <p className="text-sm text-muted-foreground">
        {FAVORITE_FILTERED_EMPTY_STATE_LABELS.description}
      </p>
      <button
        type="button"
        onClick={onReset}
      ...
      >
        {FAVORITE_FILTERED_EMPTY_STATE_LABELS.resetLabel}
      </button>
    </div>
  );
}
