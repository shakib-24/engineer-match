...
import {
  ADMIN_LOGIN_HISTORY,
  ADMIN_SECURITY_SETTINGS_SECTION,
  ADMIN_SETTINGS_DEMO_NOTE,
} from "@/constants/admin-settings";

export function AdminSecuritySettings() {
  const [twoFactorEnabled, setTwoFactorEnabled] = useState<boolean>(
    ADMIN_SECURITY_SETTINGS_SECTION.twoFactor.defaultChecked,
  );
  const [demoMessage, setDemoMessage] = useState<string | null>(null);

  function showDemoNote() {
    setDemoMessage(ADMIN_SETTINGS_DEMO_NOTE);
    window.setTimeout(() => setDemoMessage(null), 3000);
  }

  return (
    <SectionCard
      title={ADMIN_SECURITY_SETTINGS_SECTION.title}
      description={ADMIN_SECURITY_SETTINGS_SECTION.description}
    >
      <div className="flex flex-col divide-y divide-border">
        <div className="flex flex-wrap items-center justify-between gap-4 py-4 first:pt-0">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-foreground">
              {ADMIN_SECURITY_SETTINGS_SECTION.passwordChange.label}
            </p>
            <p className="mt-0.5 text-xs text-muted-foreground">
              {ADMIN_SECURITY_SETTINGS_SECTION.passwordChange.description}
            </p>
          </div>
          <button
            type="button"
          ...
          >
            {ADMIN_SECURITY_SETTINGS_SECTION.passwordChange.buttonLabel}
          </button>
        </div>

        <div className="flex items-center justify-between gap-4 py-4">
          <div className="min-w-0">
            <Label htmlFor="admin-security-2fa" className="text-foreground">
              {ADMIN_SECURITY_SETTINGS_SECTION.twoFactor.label}
            </Label>
            <p className="mt-0.5 text-xs text-muted-foreground">
              {ADMIN_SECURITY_SETTINGS_SECTION.twoFactor.description}
            </p>
          </div>
          <Switch
            id="admin-security-2fa"
        ...
        <div className="py-4">
          <p className="text-sm font-semibold text-foreground">
            {ADMIN_SECURITY_SETTINGS_SECTION.loginHistory.label}
          </p>
          <p className="mt-0.5 text-xs text-muted-foreground">
            {ADMIN_SECURITY_SETTINGS_SECTION.loginHistory.description}
          </p>
          <ul className="mt-3 flex flex-col gap-2.5">
            {ADMIN_LOGIN_HISTORY.map((item) => (
              <li
                key={item.id}
              ...
              >
                ...
                {item.isCurrent && (
                  <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-green-50 px-2.5 py-1 text-xs font-semibold text-green-700">
                    ...
                    {ADMIN_SECURITY_SETTINGS_SECTION.loginHistory.currentLabel}
                  </span>
                )}
              </li>
            ))}
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 py-4 last:pb-0">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-foreground">
              {ADMIN_SECURITY_SETTINGS_SECTION.sessionManagement.label}
            </p>
            <p className="mt-0.5 text-xs text-muted-foreground">
              {ADMIN_SECURITY_SETTINGS_SECTION.sessionManagement.description}
            </p>
          </div>
          <button
            type="button"
            ...
            className="inline-flex h-9 shrink-0 items-center justify-center rounded-xl border border-border bg-surface px-4 text-sm font-semibold text-foreground transition-colors duration-200 hover:bg-muted focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:outline-none"
          >
            {ADMIN_SECURITY_SETTINGS_SECTION.sessionManagement.revokeAllLabel}
          </button>
        </div>
      </div>
