...
import { WITHDRAW_DIALOG_LABELS } from "@/constants/applications";
...
export function WithdrawDialog({ isOpen, onCancel, onConfirm }: WithdrawDialogProps) {
  const titleId = useId();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <button
        type="button"
        aria-label={WITHDRAW_DIALOG_LABELS.cancelLabel}
        onClick={onCancel}
      ...
      >
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {WITHDRAW_DIALOG_LABELS.cancelLabel}
          </button>
          <button
            type="button"
          ...
          >
            {WITHDRAW_DIALOG_LABELS.confirmLabel}
          </button>
        </div>
      </div>
    </div>
