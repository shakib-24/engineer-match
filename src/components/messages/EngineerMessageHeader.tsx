...
import { ENGINEER_MESSAGE_HEADER_LABELS } from "@/constants/engineer-messages";

interface EngineerMessageHeaderProps {
  companyName: string;
...
export function EngineerMessageHeader({
  companyName,
  opportunityTitle,
}: EngineerMessageHeaderProps) {
  return (
    <div className="flex shrink-0 items-center gap-3 border-b border-border bg-surface px-4 py-3 sm:px-6">
      <Link
        href="/messages"
        aria-label={ENGINEER_MESSAGE_HEADER_LABELS.backLabel}
      ...
      <div className="min-w-0 flex-1">
        <h2 className="truncate text-sm font-semibold text-foreground">{companyName}</h2>
