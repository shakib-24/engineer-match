...
import { FilterFields } from "@/components/jobs/FilterSidebar";
import { FILTER_DRAWER_LABELS, type FilterState } from "@/constants/jobs";
...
export function FilterDrawer({
  ...
  return (
    ...
    >
      <button
        type="button"
        aria-label={FILTER_DRAWER_LABELS.closeLabel}
        tabIndex={isOpen ? 0 : -1}
        onClick={onClose}
        className={cn(
      ...
      <div
        ...
        aria-modal="true"
        aria-hidden={!isOpen}
        aria-label={FILTER_DRAWER_LABELS.title}
        className={cn(
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          <FilterFields filters={filters} onChange={onChange} idPrefix="drawer" />
        </div>
        ...
        <div className="shrink-0 border-t border-border p-4">
          ...
          >
            {FILTER_DRAWER_LABELS.applyLabel}（{resultCount}）
          </button>
        </div>
      </div>
    </div>
